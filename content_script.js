(async () => {
  function GetPageType() {
    ...
    if (
      URL.includes("firefly.adobe.com/generate/inpaint") ||
      URL.includes("firefly.adobe.com/upload/inpaint")
    ) {
      Type = "genfill";
    }
  ...
  if (PageType == "incorrectsite") {
    var Confirmed = window.confirm(
      "This script only works on https://firefly.adobe.com/, do you want to go there?"
    );
    if (Confirmed) {
  ...
  if (PageType == "recolorimages" || PageType == "text2image") {
    ...
    window.alert(
      "You need to allow this website to download multiple files to download all of the images if you haven't already."
    );
  ...
  if (PageType == "genfill") {
    ...
    function GetCanvas() {
      return document
        .getElementsByTagName("clio-inpaint")[0]
        .shadowRoot.firstElementChild.querySelector("clio-md-stage")
        .shadowRoot.querySelector("clio-md-compositor-canvas").shadowRoot
        .firstElementChild;
    }
